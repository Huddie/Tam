...
from tam.portfolio.costs import BpsCost, CostModel
...
def test_zero_cost_is_the_default_and_changes_nothing():
    ...
    portfolio.execute(
        Order(ticker="AAPL", side=Side.BUY, qty=10, portfolio="main"), qty=10, price=100.0, as_of=date(2024, 1, 1)
    )
...
def test_bps_cost_reduces_cash_on_a_sell_too():
    ...
    portfolio.execute(
        Order(ticker="AAPL", side=Side.SELL, qty=10, portfolio="main"), qty=10, price=100.0, as_of=date(2024, 1, 2)
    )
