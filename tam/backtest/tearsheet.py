...
--------------------------------
TearsheetChart (an alias for tam.charting.Chart) is callable -- pass a
series directly and get back a ChartCall that renders inline in a Jupyter
cell or via .show(). Chain multiple charts with | to produce a single
composite Plotly figure. See tam.charting's own docstring for the full
pattern (ChartCall/ChartPipeline, the accepted `series` shapes, and
timeseries() -- the general-purpose "plot raw series, no return/drawdown
normalization" entry point, which lives there since it has nothing to do
with a backtest tearsheet specifically):
